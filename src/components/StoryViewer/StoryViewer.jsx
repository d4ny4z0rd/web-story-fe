...
const StoryViewer = (props) => {
		...
		const fetchBookmarkStatus = async () => {
			try {
				const response = await fetch(
					`https://web-story-api.onrender.com/api/user/isBookmarked/${slides[currentSlideIndex]._id}`,
					{
						method: "GET",
						headers: {
	...
	const handleBookmark = async (slideIndex) => {
		try {
			...
			const endpoint = bookmarkStatus[slideIndex]
				...
				: "addBookmark";
			const response = await fetch(
				`https://web-story-api.onrender.com/api/user/${endpoint}`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	const handleLike = async (slideIndex) => {
		try {
			const slideId = slides[slideIndex]._id;
			const response = await fetch(
				`https://web-story-api.onrender.com/api/user/like`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	const handleShare = (slideIndex) => {
		const link = `https://web-story-fe.vercel.app/?slide=true&id=${slides[slideIndex]._id}`;
